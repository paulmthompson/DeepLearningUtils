...
def test_positional_embedding_key_save_load(tmp_path):
    """Test that RelativePositionalEmbedding2DKey can be saved and loaded with non-default parameters."""
    ...
    key_input = keras.Input(shape=(heads, key_seq_len * key_height * key_width, query_dim))
    scores_input = keras.Input(shape=(heads, query_seq_len * query_height * query_width, key_seq_len * key_height * key_width))

    output = keras_layer([key_input, scores_input])
    model = keras.Model(inputs=[key_input, scores_input], outputs=output)

    # Create test inputs
    input_key = np.random.rand(batch_size, heads, key_seq_len * key_height * key_width, query_dim).astype(np.float32)
    ...
    original_output = model.predict([input_key, input_scores])

    # Save the model
    model_path = tmp_path / "positional_embedding_key_model.keras"
    ...
    loaded_output = loaded_model.predict([input_key, input_scores])
    ...
    for layer in loaded_model.layers:
        if isinstance(layer, KerasRelativePositionalEmbedding2DKey):
            loaded_layer = layer
            break

    assert loaded_layer is not None, "RelativePositionalEmbedding2DKey layer not found in loaded model"
